...
from utils.sdf2graph import get_atom_feature_dims, get_bond_feature_dims
from .dmcg.conv import DropoutIfTraining, MLP, MLPwoLastAct, MetaLayer, GatedLinear
from .dmcg.utils import get_random_rotation_3d, clip_norm
...
class DMCG(nn.Module):
    ...
    def forward(self, batch, sample=False):
        ...
        x_embed = self.encoder_node(onehot_x.float())
        edge_attr_embed = self.encoder_edge(onehot_edge_attr.float())
    ...
    def move2origin(self, pos, batch):
        ...
        return pos - pos_mean[batch.batch]
        ...
        with torch.no_grad():
            ...
            for i in range(batch.num_graphs):
                ...
                if len(current_isomorphisms) == 1:
                    ...
                    min_idx = DMCG.alignment_loss_iso_onegraph(
                        pos_y_i,
                        pos_x_i,
                        pos_y_mean,
                        pos_x_mean,
...
def one_hot_atoms(atoms):
    ...
    #     )
    vocab_sizes = sum(get_atom_feature_dims())
    one_hot = F.one_hot(atoms, num_classes=vocab_sizes)
...
def one_hot_bonds(bonds):
    ...
    #     )
    vocab_sizes = sum(get_bond_feature_dims())
    one_hot = F.one_hot(bonds, num_classes=vocab_sizes)
    return torch.sum(one_hot, dim=1)
